...
namespace GIS.Common.Dialogs
{
    ...
    public partial class AttributeCalculator : Form
    {
        ...
        public AttributeCalculator()
        {
            ...
            this.linkLabel1.Text = StringParser.Parse("${res:GIS.Common.Dialogs.FieldCalculator.LinkLabel}");
            this.lblFunctions.Text = StringParser.Parse("${res:GIS.Common.Dialogs.FieldCalculator.Function}");
            this.lblFieldTitle.Text = StringParser.Parse("${res:GIS.Common.Dialogs.FieldCalculator.FieldTitle}");
            this.lblDestinationFieldTitle.Text = StringParser.Parse("${res:GIS.Common.Dialogs.FieldCalculator.DestinationFieldTitle}");
            this.lblComputaion.Text = StringParser.Parse("${res:GIS.Common.Dialogs.FieldCalculator.ComputationEquation}");
            this.btnClaculate.Text = StringParser.Parse("${res:GIS.Common.Dialogs.FieldCalculator.Calculate}");
            this.btnClose.Text = StringParser.Parse("${res:GIS.Common.Dialogs.FieldCalculator.Close}");
            #endregion
        ...
        private void ParExp(string exp)
        {
                    ...
                    case ("}"):
                        {
                            ...
                            if (numPart < 0)
                                throw new ArgumentException(StringParser.Parse("${res:GIS.Common.Dialogs.SymbologyFormsMessageStrings.AttributeCalculator_TooManyClosingBrackets}"));
                        }
                        break;
            ...
            for (int i = 0; i < _fs.DataTable.Rows.Count; i++)
            {
                ...
                if (noOfArr > 1)
                {
                    for (int a = 0; a < noOfArr; a++)
                    {
                        ...
                        if (templist1[a].NoOfArg == 1)
                        {
                            ...
                            {
                                throw new ArgumentException(StringParser.Parse("${res:GIS.Common.Dialogs.SymbologyFormsMessageStrings.AttributeCalculator_ParameterInvalid}"));
                            }
                ...
                {
                    throw new ArgumentException(StringParser.Parse("${res:GIS.Common.Dialogs.SymbologyFormsMessageStrings.AttributeCalculator_IncompatibleDestinationField}"));
                }
        ...
        private static double EvaluateFunction(int index, double a, double b)
        {
            ...
            switch (index)
            {
                ...
                case 66:
                    {
                        ...
                        else
                        {
                            ...
                            if ((b % 2) != 0)
                                ...
                                throw new Exception(StringParser.Parse("${res:GIS.Common.Dialogs.SymbologyFormsMessageStrings.AttributeCalculator_RootNthRootIncorrect}"));
                        }
                    }
                    break;
                ...
                case 70: result = RoundFunction(a, b);
                    break;
                default: throw new Exception(StringParser.Parse("${res:GIS.Common.Dialogs.SymbologyFormsMessageStrings.AttributeCalculator_NonStandardFunction}"));
            }
        ...
        {
            if (n < 0) throw new ArgumentException(StringParser.Parse("${res:GIS.Common.Dialogs.SymbologyFormsMessageStrings.AttributeCalculator_FactGreaterThanZero}"));
            if (n == 1.0 || n == 0.0)
            {
                return 1.0;
            }
